...
**SQL-инъекция** — внедрение вредоносного SQL через непроверенный пользовательский ввод, позволяющее читать, изменять и удалять данные в обход бизнес-логики. Защита — параметризованные запросы и ORM; запросы никогда не строятся конкатенацией строк.

**XSS (Cross-Site Scripting)** — внедрение исполняемого JavaScript в страницу, которую видят другие пользователи. Виды:

- *Stored* — скрипт сохранён на сервере, например в комментарии.
- *Reflected* — скрипт приходит в ответе на конкретный запрос, часто через query-параметр.
- *DOM-based* — уязвимость целиком в клиентском коде, без участия сервера: например, `location.hash` попадает в `innerHTML`.

**CSRF (Cross-Site Request Forgery)** — злоумышленник заставляет браузер жертвы отправить запрос к другому сайту, используя её действующие cookie-сессии (скрытая форма, автоматически отправляющая POST на банковский сайт). Защита — CSRF-токены, проверка `Origin`/`Referer` и атрибут cookie `SameSite`.
...
CORS — механизм браузера, контролируемо ослабляющий Same-Origin Policy (по умолчанию скрипт с одного origin не может читать ответы от другого). Сервер заголовками явно указывает, каким origin-ам, с какими методами и заголовками разрешено к нему обращаться из браузера.

Два сценария выполнения запроса:

- **Простые запросы** (GET/POST с базовыми заголовками) — браузер сразу выполняет запрос и проверяет в ответе `Access-Control-Allow-Origin`.
- **Непростые запросы** (кастомные заголовки, методы PUT/DELETE, `Content-Type: application/json`) — браузер сначала шлёт preflight методом `OPTIONS` и только при разрешающем ответе выполняет реальный запрос.

Важно понимать назначение: CORS защищает не сервер от клиента, а пользователя — от того, чтобы вредоносный сайт читал ответы чужого API через его браузерную сессию. Сам по себе API он не защищает: прямые обращения не из браузера (curl, Postman) CORS не ограничивает.
