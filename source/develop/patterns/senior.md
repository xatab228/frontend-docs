...
**ООП** группирует данные и поведение вокруг объектов и хорошо моделирует предметную область с изменяемым состоянием и сложными иерархиями сущностей — например, UI-компоненты с жизненным циклом. Обратная сторона: изменяемое состояние, разбросанное по множеству объектов, усложняет рассуждение о программе, повышает риск побочных эффектов и затрудняет параллелизм.

**ФП** строится вокруг чистых функций и неизменяемых данных. Это даёт предсказуемое поведение, упрощает тестирование (нет скрытого состояния) и параллельное выполнение (нет гонок за общими данными). Минусы — менее интуитивное моделирование сущностей с богатым жизненным циклом и дополнительные аллокации из-за создания новых объектов вместо мутации.

На практике фронтенд-стек смешивает оба подхода: компоненты организованы как сущности с состоянием и жизненным циклом (ООП-подобная модель), а обработка данных внутри них (`map`/`filter`/`reduce`, селекторы) написана в ФП-стиле.
