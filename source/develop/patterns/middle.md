...
SOLID — набор из пяти принципов ООП-дизайна: **S**ingle Responsibility (у класса должна быть одна причина для изменения), **O**pen/Closed (код открыт для расширения, но закрыт для модификации — новую функциональность добавляют через новые классы, а не правки существующих), **L**iskov Substitution (объект подкласса должен быть взаимозаменяем с объектом базового класса без нарушения корректности программы), **I**nterface Segregation (лучше несколько узких интерфейсов, чем один widely-used "толстый"), **D**ependency Inversion (зависеть нужно от абстракций, а не от конкретных реализаций). DRY (Don't Repeat Yourself) — избегать дублирования логики, вынося повторяющийся код в переиспользуемые функции/модули. KISS (Keep It Simple, Stupid) — предпочитать простое решение сложному, если оно решает задачу не хуже. YAGNI (You Aren't Gonna Need It) — не добавлять функциональность "про запас", пока она реально не понадобилась, чтобы не усложнять кодовую базу заранее.
...
TDD (Test-Driven Development) — разработка начинается с написания падающего теста, затем пишется минимальный код, чтобы он прошёл, затем код рефакторится (цикл red-green-refactor); фокус на корректности реализации через юнит-тесты. BDD (Behavior-Driven Development) — расширение TDD, где сценарии описываются в человекочитаемой форме (`Given/When/Then`), понятной не только разработчикам, но и продакт-менеджерам/QA, что смещает фокус с "как реализовано" на "как система себя ведёт". API First — сначала проектируется и согласовывается контракт API (например, спецификация OpenAPI), и только потом пишется реализация — это позволяет фронтенду и бэкенду разрабатываться параллельно на основе одного контракта. Code First — обратный подход, когда API-контракт генерируется из уже написанного кода (например, из декораторов контроллера) — быстрее для старта, но сложнее синхронизировать с внешними потребителями до релиза.
