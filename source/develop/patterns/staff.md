...
GRASP (General Responsibility Assignment Software Patterns) — набор принципов распределения ответственности между классами/модулями, дополняющий SOLID. Ключевые принципы: **Information Expert** (ответственность нужно поручать классу, у которого есть данные для её выполнения), **Creator** (класс A должен создавать экземпляры класса B, если A агрегирует, содержит или тесно использует B), **Controller** (входная точка системных операций делегируется одному координирующему объекту, а не размазывается по UI), **Low Coupling** (минимизировать зависимости между классами, чтобы изменения не каскадировались), **High Cohesion** (все обязанности класса должны быть тесно связаны с одной ответственностью), **Polymorphism** (варианты поведения по типу выражаются через полиморфизм, а не через `if/switch` по типу), **Pure Fabrication** (когда ни один "естественный" доменный класс не подходит для обязанности — создаётся искусственный класс-сервис ради высокой связности), **Indirection** (вводится промежуточный объект, чтобы избежать прямой связанности между компонентами), **Protected Variations** (нестабильные точки системы изолируются за стабильным интерфейсом).
