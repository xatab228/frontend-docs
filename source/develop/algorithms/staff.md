...
Пример: собственная структура для быстрого поиска ближайшей точки на карте (используется, например, в UI с геолокацией) — сочетание идей из разделов про деревья и производительность. Наивный перебор всех точек — O(n) на каждый запрос; для многократных запросов эффективнее k-d дерево, дающее в среднем O(log n):
...
Комбинация token bucket из раздела ЗНАЕТ с очередью отложенных запросов — практичный паттерн для клиента, который не должен терять запросы при превышении лимита, а откладывать их до появления токенов:
...
Полный цикл на staff-уровне: бенчмарк на реалистичных данных (не синтетических), профилирование для локализации узкого места, целевая оптимизация, повторный замер для подтверждения эффекта:
