...
Сортировка пузырьком (bubble sort) многократно проходит по массиву, меняя местами соседние элементы, если они стоят в неправильном порядке — O(n²), простая, но неэффективная на больших данных. Шейкерная сортировка (cocktail sort) — модификация пузырьковой, проходящая попеременно в обе стороны, что немного ускоряет сходимость на частично отсортированных данных. Сортировка расчёской (comb sort) улучшает пузырьковую за счёт сравнения элементов с уменьшающимся промежутком (gap), а не только соседних, что убирает "черепах" (маленькие значения в конце массива). Сортировка вставками (insertion sort) строит отсортированную часть массива слева направо, вставляя каждый следующий элемент на нужную позицию — эффективна на почти отсортированных данных, O(n²) в среднем. Сортировка выбором (selection sort) на каждом проходе находит минимальный элемент в оставшейся части и меняет его местами с текущей позицией — всегда O(n²), но делает минимум перестановок.
