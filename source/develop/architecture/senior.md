...
**MVP (Model-View-Presenter)** похож на MVC, но View полностью пассивна: вся UI-логика в Presenter, который явно вызывает методы View через интерфейс. Это упрощает unit-тестирование презентационной логики без реального рендера.

**CQRS (Command Query Responsibility Segregation)** разделяет модели чтения и записи: команды (изменяющие состояние) и запросы (читающие данные) обрабатываются разными путями, поэтому каждую сторону можно оптимизировать и масштабировать независимо. Минус — усложнение системы и возможная временная рассинхронизация read/write моделей.

**Saga** координирует распределённую транзакцию как последовательность локальных транзакций с компенсирующими действиями при сбое. Паттерн реализуется на бэкенде, но фронтенду важно его понимать, чтобы корректно показывать промежуточные и неполные состояния заказа в UI.

**Pub-Sub** связывает компоненты через шину событий: publisher публикует событие, subscriber на него реагирует, и напрямую друг о друге они не знают. Связанность падает, но отследить поток данных становится сложнее.

**Event Sourcing** хранит состояние не как текущий снимок, а как последовательность событий, из которых оно восстанавливается воспроизведением. Даёт полную историю изменений (удобно для undo/redo в сложных UI-редакторах), но усложняет чтение текущего состояния и требует снапшотов для производительности.

**Microkernel (plug-in architecture)** — минимальное стабильное ядро плюс независимо подключаемые плагины. Типичен для расширяемых приложений (IDE, редакторы): плюс — гибкость расширения, минус — сложность версионирования контрактов плагинов.
