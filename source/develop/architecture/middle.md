...
**Монолит** — вся функциональность в едином деплоймент-юните. Прост в разработке и отладке на старте, но с ростом команды и кодовой базы усложняются сборка, тестирование и деплой, а изменения в одной части рискуют затронуть другие.

**SOA (Service-Oriented Architecture)** — система из крупных сервисов, взаимодействующих через шину сообщений или общий протокол (часто SOAP/XML), с акцентом на переиспользование сервисов между приложениями организации. Со временем шина часто становится узким местом и единой точкой отказа.

**Микросервисы** — более мелкая гранулярность: каждый сервис владеет своими данными, деплоится независимо и общается лёгкими протоколами (REST/gRPC/events) без центральной шины. Дают независимость команд и отказоустойчивость, но требуют зрелой инфраструктуры (service discovery, распределённый трейсинг, оркестрация) и усложняют консистентность данных.
