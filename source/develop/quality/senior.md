...
Дополняет пример CI-пайплайна из раздела выше, добавляя интеграционные тесты из первого раздела (MSW) отдельной стадией с кэшированием:
...
По результатам Lighthouse CI из раздела выше, типичная оптимизация — code splitting тяжёлого компонента, не нужного при первой загрузке:
...
E2E-тест полного пользовательского сценария на Playwright, дополняющий пирамиду тестирования из junior-раздела на вершине:
...
Настройка Quality Gate под конкретный проект (см. раздел про SonarQube выше), где пороги подбираются под зрелость кодовой базы:
...
Стратегия "strangler fig" — постепенная замена легаси-модуля новым, работающим параллельно через общий интерфейс, вместо рискованного одномоментного переписывания:
